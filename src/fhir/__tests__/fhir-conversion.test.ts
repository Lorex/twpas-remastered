...
describe('FHIR 資料轉換', () => {
  ...
  describe('Claim 的 subType 對應申報類別', () => {
    ...
    it('續用申請應該對應代碼 "2"', () => {
      const result = convertClaimToFHIR({ ...mockClaim, applyType: '續用申請' });
      expect(result.subType?.coding?.[0]?.code).toBe('2');
    });
  });
  ...
  describe('Claim 包含正確的 References', () => {
    ...
    it('Claim.enterer 應該 reference 到 Practitioner 資源', () => {
      const result = convertClaimToFHIR(mockClaim);
      expect(result.enterer?.reference).toMatch(/^urn:uuid:|Practitioner\//);
    });

    it('Claim.provider 應該 reference 到 Organization 資源', () => {
      const result = convertClaimToFHIR(mockClaim);
      expect(result.provider?.reference).toMatch(/^urn:uuid:|Organization\//);
    });
  });
  ...
  describe('Bundle 應該包含所有必要的 entry', () => {
    ...
    it('Bundle 應該包含 Patient 資源', () => {
      const result = convertBundleToFHIR(mockClaim);
      const patients = result.entry?.filter(e => e.resource?.resourceType === 'Patient');
    ...
    it('Bundle 應該包含 MedicationRequest 資源', () => {
      const result = convertBundleToFHIR(mockClaim);
      const medicationRequests = result.entry?.filter(e => e.resource?.resourceType === 'MedicationRequest');
      expect(medicationRequests?.length).toBeGreaterThanOrEqual(1);
    });
  });
  ...
  describe('轉換醫師為 FHIR Practitioner', () => {
    it('Practitioner.identifier 應該包含醫師證書字號', () => {
      const result = convertPractitionerToFHIR(mockClaim.practitioner);
      expect(result.identifier?.some(id => id.value === 'DOC001')).toBe(true);
    });
  });
  ...
  describe('轉換用藥品項為 FHIR MedicationRequest', () => {
    it('MedicationRequest.medicationCodeableConcept 應該包含健保藥品代碼', () => {
      const result = convertBundleToFHIR(mockClaim);
      const medicationRequest = result.entry?.find(e => e.resource?.resourceType === 'MedicationRequest');
      expect(medicationRequest?.resource).toBeDefined();
    });
